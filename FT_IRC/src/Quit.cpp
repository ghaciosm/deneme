...
void Server::quit(const std::string & params, Client &client) {
    ...
    for (std::vector<Room>::iterator it = this->channels.begin(); it != this->channels.end(); it++) {
        if (it->isClientInChannel(client.getFd())) {
            ...
            {
                std::cout << "Room " + it->getName() + " has been deleted", Utils::printTime();
    ...
    std::cout << "Client " ;
    Utils::printTime() ;
    std::cout << client._ip << ":" << client.getPort()<< " " << client.getNick() << " quited !" << std::endl;
