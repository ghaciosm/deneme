...
void    Server::names(const std::string & params, Client &fd){
    ...
    if (params[0] != '#'){
        ...
        std::vector<std::string> splitted = Utils::ft_split(params, ",");
        for (vector<string>::iterator it = splitted.begin(); it != splitted.end(); it++){
            bool found = false;
            for (std::vector<Room>::iterator it2 = channels.begin(); it2 != channels.end(); it2++){
                if ((*it2).getName() == *it){
                    string message;
                    for (std::vector<Client>::iterator it3 = it2->getClients().begin(); it3 != it2->getClients().end(); it3++){
