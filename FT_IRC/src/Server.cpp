...
void Server::run()
{
	...
	while (true)
	{
		...
		if (FD_ISSET(this->_socket, &this->readFdsCopy))
		{
			...
			Utils::printTime() ;
			std::cout << newClient._ip << ":" << newClient.getPort() << std::endl;
			isReadyToSelect = true;
			continue;
		}
		for (std::vector<Client>::iterator a = clients.begin(); a != clients.end() && !isReadyToSelect; a++)
		{
			if (FD_ISSET(a->getFd(), &this->readFdsCopy))
			{
				...
				if (bytesRead <= 0)
				{
					...
					std::cout << "Client " ;
					Utils::printTime() ;
					std::cout << a->_ip << ":" << a->getPort() << " disconnected" << std::endl;
					clients.erase(a);
					isReadyToSelect = true;
				}
				else
...
{
	std::cout << "----------------\n" ;
	std::cout << "Input is : " + command + "\n";

	std::vector<std::string> softSplit = Utils::ft_split(command, "\n");
	...
	for (size_t i = 0; i < softSplit.size(); i++)
	{
		...
		std::vector<std::string> splitFirst = Utils::ft_firstWord(trimmedLine);
...
void Server::responseAllClientResponseToGui(Client &client, Room &room)  {
	string message = "";
	for (std::vector<Client>::iterator it = room.getClients().begin(); it != room.getClients().end(); it++){
...
Room	&Server::getRoom(const std::string & roomName){
...
bool	Server::isRoom(const std::string & roomName){
	std::vector<Room>::iterator it = this->channels.begin();
	for (; it != this->channels.end(); ++it)
	{
		if (it->getName() == roomName)
...
bool	Server::isClientInRoom(Room &room, const Client &client){
	std::vector<Client>::iterator it = room.getClients().begin();
...
bool	Server::isClientInRoom(Client &client, string &room){
	...
	for (; it != this->channels.end(); ++it)
	{
		if (it->getName() == room)
		{
			std::vector<Client>::iterator cit = it->getClients().begin();
