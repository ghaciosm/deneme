...
void Server::part(const std::string & params, Client &client)
{
	...
	std::vector<std::string> param = Utils::ft_split(params, " ");
	if (param[0][0] != '#'){
		param[0] = "#" + param[0];
	}
	if (isRoom(param[0])){
		...
		{
			std::cout << "Room " + room.getName() + " has been deleted", Utils::printTime();
