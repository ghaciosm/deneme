...
Bot::Bot(const std::string & port, const std::string & password) : _port(0), _password(password)
{
	this->_name = "turco";
	try
	{
...
void Bot::initSocket()
{
	...
	{
		TextEngine("Socket created successfully", cout) << std::endl;
	}
...
void	Bot::run(){
	std::vector<std::string>	turkishAnthem = getAnthem();
	...
	while (true){
		...
		if (ret < 0){
			...
			if (bytesRead < 0){
				...
				_buffer[bytesRead] = '\0';
				std::vector<std::string> messages = Utils::ft_split(_buffer, " ");
