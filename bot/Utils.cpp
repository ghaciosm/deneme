...
using namespace std;
#define std::vector<std::string> std::vector<std::string>
...
string	Utils::ft_trim(const string& str, const std::string & delims){
...
std::vector<std::string>	Utils::ft_split(const string& str, const std::string & delims){
...
std::vector<std::string>	Utils::ft_firstWord(const std::string & str)
{
	std::vector<std::string>	result;
	size_t		start = 0;
	size_t		end = 0;
...
std::string	Utils::ft_join(C_std::vector<std::string>_R vec, const std::string & delim, int start)
{
	std::string result = "";
...
int	Utils::getMaxFd(const std::vector<Client>& vec,int server){
	if (vec.size() == 0){
		return (server);
	}
	int		max = vec[0].getFd();
